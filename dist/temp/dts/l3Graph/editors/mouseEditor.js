...
var MouseEditor = /** @class */ (function () {
    ...
    MouseEditor.prototype.onOverlayDown = function (event, target) {
        var _this = this;
        if (graphModel_1.isLink(target)) {
            return;
        }
        var nodeTreePos = utils_1.vector3DToTreeVector3(target.position);
        var cameraPos = this.diagramView.camera.position;
        var distanceToNode = nodeTreePos.distanceTo(cameraPos);
        this.diagramhModel.selection = new Set([target]);
        this.arrowHelper.focusNode = target;
        var onWheel = function (wheelEvent) {
            var e = wheelEvent;
            distanceToNode -= (e.deltaX || e.deltaY || e.deltaZ) * WHEEL_SPEED;
            wheelEvent.stopPropagation();
            target.position = _this.diagramView.mouseTo3dPos(e, distanceToNode);
        };
        document.body.addEventListener('mousewheel', onWheel);
        utils_1.handleDragging(event, function (dragEvent) {
            target.position = _this.diagramView.mouseTo3dPos(dragEvent, distanceToNode);
        }, function () {
            _this.arrowHelper.focusNode = undefined;
