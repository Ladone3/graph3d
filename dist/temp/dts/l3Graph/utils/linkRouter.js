...
var DefaultLinkRouter = (function () {
    ...
    DefaultLinkRouter.prototype.getLinkGroup = function (link) {
    ...
    DefaultLinkRouter.prototype.getRout = function (link) {
        var group = this.getLinkGroup(link);
        var sourcePos = link.source.position;
        var targetPos = link.target.position;
        var median = geometry_1.multiply(geometry_1.sum(sourcePos, targetPos), 0.5);
        if (group.length === 1) {
            ...
            var kinkPoint = geometry_1.sum(median, offset);
