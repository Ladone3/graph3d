...
function createSprite(htmlOverlay, position) {
    ...
    sprite.center = CENTER_VECTOR_FOR_POSITION[position];
    texture.wrapS = texture.wrapT = THREE.MirroredRepeatWrapping;
    return htmlToImage(htmlOverlay).then(function (img) {
        texture.image = img;
...
function htmlToImage(htmlElement) {
    var img = _htmlToImage(htmlElement);
    return new Promise(function (resolve) {
        img.onload = function () { resolve(img); };
    });
}
exports.htmlToImage = htmlToImage;
function _htmlToImage(htmlElement) {
    var img = new Image();
    if (!htmlElement) {
        img.src = ERROR_BASE_64;
        return img;
    }
    domtoimage.toPng(htmlElement).then(function (dataUrl) {
        img.src = dataUrl;
...
var CENTER_VECTOR_FOR_POSITION = {
