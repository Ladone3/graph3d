 ...
 * Didn't have anough time to properly transfer from js to ts,
...
type Group = THREE.Group & {materialLibraries?: any[]};

function createEmptyMaterial(name: string, smooth: any) {
...
class ObjObject {
	...
	startMaterial(name: string, libraries: any) {
		const previous = this.finalize(false);
...
class ParserState {
	public objects: ObjObject[] = [];
	public object: ObjObject;
	public vertices: any[] = [];
	public normals: any[] = [];
	public colors: any[] = [];
	public uvs: any[] = [];
	public materialLibraries: any[] = [];
	...
	addLineGeometry(vertices: any[], uvs: string[]) {
...
export class OBJLoader {
	private materials: THREE.Material[] & { create: (name: string) => THREE.Material};
	private path: string;
	...
	parse(text: string) {
		...
		const trimLeft = ((typeof '' as any).trimLeft === 'function');

		for (let i = 0, l = lines.length; i < l; i ++) {
			line = lines[i];
			line = trimLeft ? (line as any).trimLeft() : line.trim();
		...
		const container: Group = new THREE.Group();
			...
			for (let mi = 0, miLen = materials.length; mi < miLen; mi ++) {
				...
				if (this.materials !== null) {
					...
					if (isLine && material && !(material instanceof THREE.LineBasicMaterial)) {
						...
						(materialPoints as any).lights = false;
